...
$probeBuild = Join-Path $probeRepo 'build/jk-runtime/_INTERNAL/frame-probe'
...
$probeSource = Join-Path $PSScriptRoot 'FrameProbe.cs'
& $probeCompiler /nologo /optimize+ /target:library /langversion:5 /nowarn:1685 "/out:$probeBuild/JKFrameProbe.dll" $probeReferences $probeSource
if ($LASTEXITCODE -ne 0) { throw 'Frame probe compilation failed' }
...
if ($LASTEXITCODE -ne 0) { throw 'Frame probe fixture compilation failed' }
& "$probeBuild/ProbeTests.exe"
if ($LASTEXITCODE -ne 0) { throw 'Frame probe checks failed' }
Write-Host "[OK] Temporary diagnostic only; not installed: $probeBuild/JKFrameProbe.dll"
